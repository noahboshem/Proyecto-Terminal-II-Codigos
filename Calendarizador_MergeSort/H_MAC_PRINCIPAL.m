...
N         =20; %NUMERO DE NODOS POR GRADO
m         =N;
W         =m;%CANTIDAD MAXIMA DE RANURAS
Lambda    =0.009;%TASA DE GENERACION DE PAQUETES
...
%%---------------------INICIALIZAMOS NODOS-----------------------------
nodos = struct('buffer',{}); %%DEFINIMOS LA ESTRUCTURA DE LOS NODOS
for i=1:I
for n = 1:N
nodos(i,n).buffer = []; %%%iNICIALIZAMOS EL BUFFER DE LOS NODOS 
end
end
...
for cic1=1:Ciclos   %%CORRE TANTOS CICLOS COMO DESEEMOS
    ...
    for nod=1:N     %%CORRE (#NODOS/GRADO) CICLOS
        %%%%%%%%%%%%%%%%%%%DEADLINE%%%%%%%%%%%%%%%%%%%%%%%
        nodos(i,nod).buffer=actualiza_tiempos(nodos(i,nod).buffer,T_sim); 
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    %%%%eleccion de ranuras mediante metodos hash
    raiz=cic1*grad;
    ranura=Ranuras_hash(N,raiz); %%LOS NODOS CON PAQUETES POR TRANSMITIR ELIGEN RANURA
    vacio=not(tx_flag);
    ranura(vacio)=N+5;
    %%%%%termina eleccion de ranuras mediante funcion hash
    if(any(tx_flag))
    ...
    if(length(nod)==1)%%COMPROBAMOS QUE SOLO HAYA UN NODO GANADOR PARA LA RANURA ELEGIDA
        %%%%%%%%%%%%%%%%%%%%SE ORDENA EL BUFFER DEL NODO ANTES DE TRANSMITIR%%%%%%%%%%%%%%
        v1=nodos(i,nod).buffer;
        nodos(i,nod).buffer=fliplr(MergeSort(nodos(i,nod).buffer));
        v2=nodos(i,nod).buffer;
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
avg_timeratio
pkt_loss=1-(pkts_recibidos')./(pkt_status(:,1)+pkt_status(:,2))
